 ...
 * tier. However, this creates a two-fold problem: firstly, not all spells are in tier order, and secondly, if spells are
 * added via addon mods the entries would have to be updated manually with the new numbers.
 * <p>
 ...
 * stupidly long but would allow precise control over each individual spell... I'll leave that for pack makers to do
...
public class RandomSpell extends LootFunction {
	...
	public ItemStack apply(ItemStack stack, Random random, LootContext context){
		...
		/*
		if(spells.isEmpty()){
			spellsList = Spell.getSpells(new Spell.TierElementFilter(EnumTier.BASIC, null));
			if(stack.getItem() instanceof ItemScroll) spellsList.retainAll(Spell.getSpells(Spell.nonContinuousSpells));
		}
		*/
        ...
        public RandomSpell deserialize(JsonObject object, JsonDeserializationContext deserializationContext, LootCondition[] conditions){
            ...
            if(object.has("elements")){
                ...
                for(JsonElement jelement : JsonUtils.getJsonArray(object, "elements", new JsonArray())){
                    ...
                    // IDEA: If it is necessary to get elements by name elsewhere in the future, move this to EnumElement. 
