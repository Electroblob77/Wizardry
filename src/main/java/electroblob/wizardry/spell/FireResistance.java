...
public class FireResistance extends Spell {
	...
	public boolean cast(World world, EntityPlayer caster, int ticksInUse, float damageMultiplier, float rangeMultiplier, float durationMultiplier, float blastMultiplier) {
		...
		caster.addPotionEffect(new PotionEffect(Potion.fireResistance.id, (int)(600*durationMultiplier), 0, true));
		...
		if(world.isRemote){
			for(int i=0; i<10; i++){
				...
				Wizardry.proxy.spawnParticle(EnumParticleType.SPARKLE, world, x1, y1, z1, 0, 0.1F, 0, 48 + world.rand.nextInt(12), 1.0f, 0.5f, 0.0f);
			}
		}
	...
	public boolean cast(World world, EntityLiving caster, EntityLivingBase target, float damageMultiplier, float rangeMultiplier, float durationMultiplier, float blastMultiplier) {
		...
		if(caster.isBurning() && !caster.isPotionActive(Potion.fireResistance)){
